...
class DD_xinxi():
    ...
    def reset_meau(self):
        print("确定要删除吗?\n1.确定\t\t2.取消")
        try:
            user_set = int(input("DD> "))
            if user_set == 1:
                self.DDcf.remove_section("def_xinxi")
                self.DDcf.remove_section("def_meau")
                self.DDcf.add_section("def_xinxi")
                self.DDcf.add_section("def_meau")
                try:
                    user_set = int(input("要添加多少项："))
                    for i in range(1,user_set+1):
                        try:
                            user_set_id = input("请输入名称：")
                            user_set_uid = int(input("请输入uid："))
                            self.DDcf.set("def_xinxi",user_set_id,str(user_set_uid))
                            self.DDcf.set("def_meau",str(i),user_set_id)
                        except ValueError:
                            print("请输入正确参数")
                    self.DDcf.set("def_meau","0","退出")
    ...
    def get_meau(self):
        meau = sorted(self.DDcf.items("def_meau"),key=lambda item:item[0], reverse=True)
